...
int config_write_atomic(const char*p,const char*b,size_t n){char tmp[512],bak[512];int fd;ssize_t w;if(!p||strlen(p)>450)return-1;snprintf(tmp,sizeof(tmp),"%s.tmp",p);snprintf(bak,sizeof(bak),"%s.bak",p);fd=open(tmp,O_WRONLY|O_CREAT|O_TRUNC,0600);if(fd<0)return-1;w=write(fd,b,n);if(w!=(ssize_t)n||fsync(fd)){close(fd);unlink(tmp);return-1;}if(close(fd)){unlink(tmp);return-1;}unlink(bak);if(link(p,bak)<0&&errno!=ENOENT){/* backup is best effort */}if(rename(tmp,p)){unlink(tmp);return-1;}chmod(p,0600);return 0;}
