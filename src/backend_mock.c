...
static int wake(struct le_backend*b,struct le_wake_word_state*o){if(!M(b)->wake_available)return LE_NOT_SUPPORTED;*o=M(b)->wake;o->capture_active=o->processed_frames&&le_wake_recent(o->capture_age_ms,LE_WAKE_CAPTURE_STALE_MS);o->inference_active=o->model_loaded&&le_wake_recent(o->inference_age_ms,LE_WAKE_INFERENCE_STALE_MS);return LE_OK;}static int wake_set(struct le_backend*b,const char*s){if(!M(b)->wake_available)return LE_NOT_SUPPORTED;if(!s[0]||strlen(s)>=LE_TEXT)return LE_INVALID;strcpy(M(b)->wake.wake_word,s);return save(M(b));}static int sensitivity(struct le_backend*b,int v){if(!M(b)->wake_available)return LE_NOT_SUPPORTED;if(v<0||v>100)return LE_INVALID;M(b)->wake.sensitivity=v;return save(M(b));}static int wake_test(struct le_backend*b){if(!M(b)->wake_available)return LE_NOT_SUPPORTED;if(!M(b)->wake.enabled)return LE_BUSY;M(b)->wake.detected_count++;return LE_OK;}
...
static int reboot(struct le_backend*b){struct mock_state*m=M(b);strcpy(m->power,"rebooting");m->recover_at=time(0)+3;return LE_OK;}static int shutdown(struct le_backend*b){strcpy(M(b)->power,"shutting_down");return LE_OK;}static int reset(struct le_backend*b){char p[384];unsigned seed=M(b)->rng;if(fail(M(b),"factory-reset"))return LE_IO;strcpy(p,M(b)->config_path);defaults(M(b),seed);strcpy(M(b)->config_path,p);return save(M(b));}
