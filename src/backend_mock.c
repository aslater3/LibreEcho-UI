...
struct mock_state { int integrations_spotify; char spotify_device_name[64]; struct le_audio_state audio;struct le_led_state led;struct le_network_state net;struct le_wake_word_state wake;struct le_bluetooth_state bluetooth;struct le_airplay_state airplay;struct le_playback_state playback;struct le_wifi_scan scan;char device_name[64],config_path[384],pending_ssid[64],fail_next[32],power[24];unsigned rng;time_t started,connect_at,recover_at,bt_scan_ends;int temp_base,update_progress; struct le_timer_entry timers[16];long long timer_due[16];int timer_count;unsigned next_timer_id,timer_missed;};
/* How long a mock Bluetooth discovery runs before the controller reports it
 * finished. Short enough for a browser test, long enough to observe. */
#define MOCK_BT_SCAN_SECONDS 3
...
static int wake(struct le_backend*b,struct le_wake_word_state*o){*o=M(b)->wake;return LE_OK;}static int wake_set(struct le_backend*b,const char*s){if(!s[0]||strlen(s)>=LE_TEXT)return LE_INVALID;strcpy(M(b)->wake.wake_word,s);return save(M(b));}static int sensitivity(struct le_backend*b,int v){if(v<0||v>100)return LE_INVALID;M(b)->wake.sensitivity=v;return save(M(b));}static int wake_test(struct le_backend*b){if(!M(b)->wake.enabled)return LE_BUSY;M(b)->wake.detected_count++;return LE_OK;}
static int bluetooth(struct le_backend*b,struct le_bluetooth_state*o){*o=M(b)->bluetooth;return LE_OK;}static int bluetooth_set(struct le_backend*b,int enabled){if(enabled!=0&&enabled!=1)return LE_INVALID;M(b)->bluetooth.activation_attempted=1;M(b)->bluetooth.enabled=enabled;strcpy(M(b)->bluetooth.state,enabled?"up":"off");strcpy(M(b)->bluetooth.hci,enabled?"hci0":"none");if(!enabled){M(b)->bluetooth.scanning=0;M(b)->bt_scan_ends=0;M(b)->bluetooth.discovered_count=0;}return LE_OK;}
...
/* Discovery is modelled the way the controller behaves: starting a scan clears
 * the nearby list, the scan runs for a while, and the results appear when it
 * ends. A mock that lists devices before any scan cannot show whether the UI
 * refreshes itself once a scan finishes. */
static void mock_bt_discovered(struct mock_state*m){m->bluetooth.discovered_count=2;strcpy(m->bluetooth.discovered[0].address,"10:20:30:40:50:60");strcpy(m->bluetooth.discovered[0].name,"Mock Speaker");m->bluetooth.discovered[0].type=0;m->bluetooth.discovered[0].rssi=-42;m->bluetooth.discovered[0].rssi_valid=1;strcpy(m->bluetooth.discovered[1].address,"20:30:40:50:60:70");strcpy(m->bluetooth.discovered[1].name,"Mock Headphones");m->bluetooth.discovered[1].type=0;m->bluetooth.discovered[1].rssi=-58;m->bluetooth.discovered[1].rssi_valid=1;}
static int bluetooth_scan_mock(struct le_backend*b,int start){struct mock_state*m=M(b);if(!m->bluetooth.enabled)return LE_BUSY;m->bluetooth.scanning=!!start;if(start){m->bluetooth.discovered_count=0;m->bt_scan_ends=time(0)+MOCK_BT_SCAN_SECONDS;}else m->bt_scan_ends=0;return LE_OK;}
...
/* The simulated stream reports ICY fields because a real one does; a station
   that sends nothing is simulated by the empty strings this leaves behind
   when the mock stream is stopped. */
static int radio_play_mock(struct le_backend*b,const char*url){
 if(!url||!url[0])return LE_INVALID;
 snprintf(mock_radio_url,sizeof(mock_radio_url),"%s",url);mock_radio_playing_flag=1;
 strcpy(M(b)->playback.state,"playing");strcpy(M(b)->playback.source,"radio");
 M(b)->playback.media_active=1;M(b)->playback.metadata_available=1;
 strcpy(M(b)->playback.title,"Simulated Artist - Simulated Track");
 M(b)->playback.artist[0]='\0';M(b)->playback.album[0]='\0';
 return LE_OK;}
static int radio_stop_mock(struct le_backend*b){
 mock_radio_playing_flag=0;mock_radio_url[0]='\0';
 strcpy(M(b)->playback.state,"idle");M(b)->playback.source[0]='\0';
 M(b)->playback.media_active=0;M(b)->playback.metadata_available=0;
 M(b)->playback.title[0]='\0';M(b)->playback.artist[0]='\0';M(b)->playback.album[0]='\0';
 return LE_OK;}
static int radio_playing_mock(struct le_backend*b,struct le_radio_status*o){(void)b;
...
static const struct le_backend_ops ops={destroy,status,device,audio,volume,gain,mute,tone,tts_voice,0,0,noise_start_mock,noise_stop_mock,simulate_audio_mock,radio_play_mock,radio_stop_mock,radio_playing_mock,led,colour,brightness,visualizer_enabled,boot_led,led_profile,night_mock,led_test,network,scan,connect_wifi,disconnect_wifi,hostname,wake,wake_set,sensitivity,wake_test,bluetooth,bluetooth_set,bluetooth_scan_mock,bluetooth_pair_mock,bluetooth_unpair_mock,bluetooth_disconnect_mock,bluetooth_pairing_response_mock,bluetooth_discoverable_mock,bluetooth_connectable_mock,bluetooth_pairing_mode_mock,airplay,airplay_set,playback,reboot,shutdown,reset,tick,control,
 spotify_mock, spotify_set_mock,
 /* light: the mock has no sensor; the dispatcher reports unsupported. */
 0,
