...
static int setup_marker_present(const char*config_path){char path[512];int n;if(!config_path||!config_path[0])return 0;n=snprintf(path,sizeof(path),"%s.setup-complete",config_path);return n>0&&(size_t)n<sizeof(path)&&access(path,R_OK)==0;}
static int setup_account_pending(const char*config_path){char path[512];int n;if(!config_path||!config_path[0])return 0;n=snprintf(path,sizeof(path),"%s.setup-account-created",config_path);return n>0&&(size_t)n<sizeof(path)&&access(path,R_OK)==0;}
