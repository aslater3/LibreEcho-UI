...
return 1;
}if(random_csrf(csrf,sizeof(csrf))){fprintf(stderr,"Unable to obtain secure CSRF token\n");le_backend_destroy(b);return 2;}if(api_init(&api,b,dev,insecure_lan,token,allowed_origin,csrf,cfg,users_path)){fprintf(stderr,"Unable to initialise authentication\n");le_backend_destroy(b);return 2;}api.listener_lan=strcmp(o.listen_host,"127.0.0.1")&&strcmp(o.listen_host,"::1");if(cfg&&access(cfg,F_OK)==0)api.setup_completed=1;
{int apply_rc=LE_IO,apply_try;for(apply_try=0;apply_try<12&&apply_rc;apply_try++){apply_rc=api_apply_persisted_configuration(&api);if(apply_rc)usleep(250000);}if(apply_rc)api_log(&api,"warning","One or more persisted hardware settings could not be restored");}
