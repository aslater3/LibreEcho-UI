...
/* Completion is proven by the setup transaction's own marker, and only when
   that marker carries the schema this build understands. A file that merely
   exists -- empty, truncated, or written by an unknown schema -- must leave the
   device in setup so the wizard still runs. The accepted marker forms are the
   same ones the configuration worker re-reads when it refreshes completion. */
static int setup_marker_valid(const char*config_path){char path[512],marker[32];int n;if(!config_path||!config_path[0])return 0;n=snprintf(path,sizeof(path),"%s.setup-complete",config_path);if(n<0||(size_t)n>=sizeof(path))return 0;n=config_read(path,marker,sizeof(marker));if(n<=0)return 0;return !strcmp(marker,"schema=1")||!strcmp(marker,"schema=1\n")||!strcmp(marker,"schema=1\r\n");}
