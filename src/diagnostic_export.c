...
static int append_logs(struct diag_writer *w, const struct api_context *c)
{
    ...
    for (i = 0; i < count; ++i) {
        ...
        char level[32], escaped_level[LE_DIAG_TEXT * 2];
        char message[LE_DIAG_TEXT], redacted[LE_DIAG_TEXT * 2];
        int boot_seconds = 0;
