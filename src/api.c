...
/* Monotonic milliseconds for the capture-liveness window; 0 means the clock could not be read and the caller's sample is left alone. */
static unsigned long long wake_now_ms(void){struct timespec ts;return clock_gettime(CLOCK_MONOTONIC,&ts)==0?(unsigned long long)ts.tv_sec*1000ULL+(unsigned long long)ts.tv_nsec/1000000ULL:0ULL;}
/* Capture health is a successive-sample judgement and only meaningful while a model is loaded: fold this read's frame counter into the retained sample (wake_health.h) and report the last verdict. VAD is deliberately not consulted -- a quiet room reports vad_active false while capture runs normally. */
static int wake_capture_alive(struct api_context*c,const struct le_wake_word_state*w){if(!w->model_loaded)return 0;le_wake_sample_update(&c->wake_capture,w->processed_frames,wake_now_ms());return le_wake_capture_alive(&c->wake_capture);}
static void wake_json(struct api_context*c,struct api_response*r){struct le_wake_word_state w;char wake[LE_TEXT*2],model[64];int rc=le_get_wake_word_state(c->backend,&w);if(rc==LE_NOT_SUPPORTED){out(r,200,"{\"ok\":true,\"data\":{\"available\":false,\"unavailable\":true,\"message\":\"Wake-word service is not available\"},\"error\":null}");return;}if(rc){err(r,503,rc,"Wake-word service status is unavailable");return;}json_escape(wake,sizeof(wake),w.wake_word);json_escape(model,sizeof(model),w.model_status);/* vad_active is frame content (a quiet room is normal); capture_active is the successive-sample verdict, so operators are never told capture failed during silence. processed_frames prints %llu: one 10 ms frame per tick passes INT_MAX after ~249 days on a device that is never rebooted. */int capture=wake_capture_alive(c,&w);out(r,200,"{\"ok\":true,\"data\":{\"enabled\":%s,\"wake_word\":\"%s\",\"sensitivity\":%d,\"cooldown_ms\":%d,\"model_status\":\"%s\",\"detected_count\":%d,\"cpu_cost_percent\":%d,\"memory_cost_mb\":%d,\"model_loaded\":%s,\"vad_active\":%s,\"capture_active\":%s,\"processed_frames\":%llu,\"local_processing\":true},\"error\":null}",w.enabled?"true":"false",wake,w.sensitivity,w.cooldown_ms,model,w.detected_count,w.cpu_cost,w.memory_cost_mb,w.model_loaded?"true":"false",w.vad_active?"true":"false",capture?"true":"false",w.processed_frames);}
 ...
 */
 if(!strcmp(p,"/api/v1/system/features")){
  ...
  if(!strcmp(q->method,"PUT")){int host,want_host=json_get_bool(q->body,"usb_host",&host);
   ...
   {int hv,want_https=json_get_bool(q->body,"https",&hv),want_sim=json_get_bool(q->body,"simulation",&v);
