...
static void update_status_json(struct api_context*c,struct api_response*r)
{
    ...
    out(r,200,"{\"ok\":true,\"data\":{\"supported\":%s,\"current_slot\":\"%s\","
        ...
        "\"components\":%s,\"transaction_state\":\"%s\",\"last_transaction_result\":\"%s\"},"
...
static void provenance_json(struct api_response*r){
    char components[LE_FEATURE_COMPONENTS_JSON_MAX],state[80],last_result[80];
    le_feature_transaction_state transaction;
    le_feature_transaction_state_read(&transaction);
    le_feature_components_json(components,sizeof(components),&transaction);
    json_escape(state,sizeof(state),transaction.state);
    json_escape(last_result,sizeof(last_result),transaction.last_result);
    out(r,200,"{\"ok\":true,\"data\":{\"os_version\":\"%s\",\"source_commit\":\"%s\",\"source_dirty\":%s,\"source_digest\":\"%s\",\"components\":%s,\"transaction_state\":\"%s\",\"last_transaction_result\":\"%s\"},\"error\":null}",
        LE_OS_VERSION_STRING,LE_SOURCE_COMMIT,
        !strcmp(LE_SOURCE_DIRTY,"1")?"true":"false",LE_SOURCE_DIGEST,
        components,state,last_result);
}
