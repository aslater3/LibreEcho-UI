...
static int duplicate_json_key(const char *body,const char *key){char needle[64];const char *first,*second;int n=snprintf(needle,sizeof(needle),"\"%s\"",key);if(n<0||(size_t)n>=sizeof(needle))return 1;first=strstr(body,needle);if(!first)return 0;second=strstr(first+strlen(needle),needle);return second!=NULL;}
