...
static int write_setup_account_marker(const struct api_context*c,int present){char path[512];int n;if(!c||!c->config_path[0])return LE_IO;n=snprintf(path,sizeof(path),"%s.setup-account-created",c->config_path);if(n<0||(size_t)n>=sizeof(path))return LE_IO;if(!present)return unlink(path)&&errno!=ENOENT?LE_IO:LE_OK;return config_write_atomic(path,"schema=1\n",8)?LE_IO:LE_OK;}
...
static void auth_bootstrap_json(struct api_context*c,const struct api_request*q,struct api_response*r)
{
    ...
    if(write_setup_marker(c,0)||write_setup_account_marker(c,1)){err(r,503,LE_IO,"Account was created but setup state could not be reopened");goto clear;}
    c->setup_completed=0;
