                          ...
                          struct api_response *r)
{
    ...
    if(parsed<0||(parsed>0&&!audio_remote_url_valid(remote_url))){err(r,400,LE_INVALID,"audio_remote_url must be an HTTPS URL without embedded credentials");return 1;}
    if(parsed>0)url_present=1;
    if(mode_present&&!strcmp(mode,"remote")&&!(url_present?remote_url[0]:c->privacy_audio_remote_url[0])){err(r,400,LE_INVALID,"A remote HTTPS destination is required for remote retention");return 1;}
