...
static void update_status_json(struct api_context*c,struct api_response*r)
{
    ...
    out(r,200,"{\"ok\":true,\"data\":{\"supported\":%s,\"current_slot\":\"%s\","
        ...
        allow_unsigned?"true":"false",components,escaped_transaction_state,
...
{
    return action && (!strcmp(action, "sound") || !strcmp(action, "disabled"));
}
static int button_sounds_valid(const char *sounds)
{
    const char *p, *end;
    size_t length;
    int valid;
    if (!sounds || !sounds[0])
        return sounds != NULL;
    for (p = sounds; *p; p = *end ? end + 1 : end) {
        end = strchr(p, ',');
        if (!end)
            end = p + strlen(p);
        length = (size_t)(end - p);
        valid = (length == strlen("action-1") &&
                 !strncmp(p, "action-1", length)) ||
                (length == strlen("action-2") &&
                 !strncmp(p, "action-2", length)) ||
                (length == strlen("action-3") &&
                 !strncmp(p, "action-3", length));
        if (!valid)
            return 0;
        if (!*end)
            break;
...
static int button_setting_int(const char *saved, const char *key, int *out)
{
    long long value;
    int rc = json_get_int64(saved, key, &value);
    if (rc != 1)
        return rc;
...
static void boot_control_json(struct api_response*r){
    struct le_boot_control boot;
    char payload[LE_BOOT_CONTROL_JSON_MAX],body[LE_BOOT_CONTROL_JSON_MAX+64];
    le_boot_control_read(&boot);
    if(le_boot_control_json(payload,sizeof(payload),&boot)){err(r,503,LE_IO,"Boot diagnostics are unavailable");return;}
    snprintf(body,sizeof(body),"{\"ok\":true,\"data\":%s,\"error\":null}",payload);
    out(r,200,body);
}
...
static void auth_bootstrap_json(struct api_context*c,const struct api_request*q,struct api_response*r)
{
    ...
    if(write_setup_marker(c,0)){err(r,503,LE_IO,"Account was created but setup state could not be reopened");goto clear;}
    c->setup_completed=0;
...
static void buttons_json(struct api_context *c, struct api_response *r)
{
    ...
    char escaped_short[80], escaped_long[80], escaped_action[48], escaped_sounds[384];
    struct stat st;
    ...
    json_escape(escaped_action, sizeof(escaped_action), c->button_action);
    json_escape(escaped_sounds, sizeof(escaped_sounds), c->button_action_sounds);
    ...
    out(r, 200, "{\"ok\":true,\"data\":{\"short_press\":\"%s\",\"long_press\":\"%s\",\"available\":%s,\"state\":\"%s\",\"volume_capable\":%s,\"hardware_mute\":%s,\"action_capable\":%s,\"stale\":%s,\"tones\":%s,\"action\":\"%s\",\"action_sounds\":\"%s\",\"action_brightness\":%d,\"mute_brightness\":%d},\"error\":null}", escaped_short, escaped_long, fresh && !strcmp(state, "connected") ? "true" : "false", state, volume ? "true" : "false", mute ? "true" : "false", action ? "true" : "false", !fresh ? "true" : "false", c->button_tones ? "true" : "false", escaped_action, escaped_sounds, c->button_action_brightness, c->button_mute_brightness);
}
...
void api_handle(struct api_context*c,const struct api_request*q,struct api_response*r){const char*p=q->path;int rc=LE_OK,v;if(!security(c,q,r))return;if((!strcmp(p,"/api/v1/timers")&&(strcmp(q->method,"GET")&&strcmp(q->method,"POST")))||(!strcmp(p,"/api/v1/timers/dismiss")&&strcmp(q->method,"POST"))||(!strncmp(p,"/api/v1/timers/",15)&&strcmp(p,"/api/v1/timers/dismiss")&&strcmp(q->method,"DELETE"))){method_not_allowed(r);return;}if((!strcmp(p,"/api/v1/buttons")&&strcmp(q->method,"GET")&&strcmp(q->method,"PUT"))||(!strcmp(p,"/api/v1/privacy")&&strcmp(q->method,"GET")&&strcmp(q->method,"PUT"))||(!strcmp(p,"/api/v1/integrations")&&strcmp(q->method,"GET"))||(!strncmp(p,"/api/v1/integrations/",21)&&strcmp(q->method,"PUT"))){method_not_allowed(r);return;}if(changing(q->method)&&q->body_len&&!body_ok(q,r))return;if(!strcmp(p,"/api/v1/auth/bootstrap")&&!strcmp(q->method,"POST")){auth_bootstrap_json(c,q,r);return;}if(!strcmp(p,"/api/v1/auth/login")&&!strcmp(q->method,"POST")){auth_login_json(c,q,r);return;}if(!strcmp(p,"/api/v1/auth/users")&& !strcmp(q->method,"GET")){auth_users_json(c,r);return;}if(!strcmp(p,"/api/v1/auth/users")&& !strcmp(q->method,"POST")){auth_add_user_json(c,q,r);return;}if(!strncmp(p,"/api/v1/auth/users/",strlen("/api/v1/auth/users/"))&& !strcmp(q->method,"DELETE")){auth_remove_user_json(c,q,r);return;}if((!strcmp(p,"/api/v1/auth/users")||!strncmp(p,"/api/v1/auth/users/",strlen("/api/v1/auth/users/")))&&strcmp(q->method,"GET")&&strcmp(q->method,"POST")&&strcmp(q->method,"DELETE")){method_not_allowed(r);return;}if(!strcmp(p,"/api/v1/auth")&&!strcmp(q->method,"GET")){auth_current_json(c,q,r);return;}if(!strcmp(p,"/api/v1/auth/logout")&&!strcmp(q->method,"POST")){if(!strncmp(q->authorization,"Bearer ",7))le_auth_logout(&c->auth,q->authorization+7);ok(r,"{\"logged_out\":true}");return;}
 ...
 if(!strcmp(p,"/api/v1/network/wifi/connect")&&!strcmp(q->method,"POST")){struct le_wifi_credentials w;int security_result;memset(&w,0,sizeof(w));if(json_get_string(q->body,"ssid",w.ssid,sizeof(w.ssid))<1){err(r,400,LE_INVALID,"SSID is required");return;}json_get_string(q->body,"password",w.password,sizeof(w.password));security_result=json_get_string(q->body,"security",w.security,sizeof(w.security));if(security_result<0||(security_result>0&&!valid_wifi_security(w.security))){memset(w.password,0,sizeof(w.password));err(r,400,LE_INVALID,"security must be open or wpa2");return;}if(security_result==0)strcpy(w.security,"wpa2");rc=le_connect_wifi(c->backend,&w);memset(w.password,0,sizeof(w.password));if(rc){err(r,rc==LE_INVALID?400:503,rc,"Wi-Fi connection failed");return;}event_bus_publish(&c->events,"network","{\"state\":\"connecting\"}");ok(r,"{\"state\":\"connecting\"}");return;}if(!strcmp(p,"/api/v1/network/wifi/disconnect")&&!strcmp(q->method,"POST")){rc=le_disconnect_wifi(c->backend);if(rc)err(r,501,rc,"Wi-Fi disconnect is unavailable");else ok(r,"{\"state\":\"disconnected\"}");return;}
 ...
 if(!strcmp(p,"/api/v1/buttons")){if(!strcmp(q->method,"PUT")){char short_action[32],long_action[32],action_value[24],sounds[192],next_short[32],next_long[32],next_action[24],next_sounds[192],old_short[32],old_long[32],old_action[24],old_sounds[192];int old_tones=c->button_tones,old_ab=c->button_action_brightness,old_mb=c->button_mute_brightness;int t=0,ab=0,mb=0,field,changed=0,next_tones=c->button_tones,next_action_brightness=c->button_action_brightness,next_mute_brightness=c->button_mute_brightness;snprintf(next_short,sizeof(next_short),"%s",c->button_short);snprintf(next_long,sizeof(next_long),"%s",c->button_long);snprintf(next_action,sizeof(next_action),"%s",c->button_action);snprintf(next_sounds,sizeof(next_sounds),"%s",c->button_action_sounds);field=json_get_bool(q->body,"tones",&t);if(field<0){err(r,400,LE_INVALID,"Button settings are invalid");return;}if(field>0){next_tones=t?1:0;changed=1;}field=json_get_string(q->body,"action",action_value,sizeof(action_value));if(field<0||(field>0&&!button_action_valid(action_value))){err(r,400,LE_INVALID,"Button settings are invalid");return;}if(field>0){snprintf(next_action,sizeof(next_action),"%s",action_value);changed=1;}field=json_get_string(q->body,"action_sounds",sounds,sizeof(sounds));if(field<0||(field>0&&!button_sounds_valid(sounds))){err(r,400,LE_INVALID,"Button settings are invalid");return;}if(field>0){snprintf(next_sounds,sizeof(next_sounds),"%s",sounds);changed=1;}field=button_setting_int(q->body,"action_brightness",&ab);if(field<0){err(r,400,LE_INVALID,"Button settings are invalid");return;}if(field>0){next_action_brightness=ab;changed=1;}field=button_setting_int(q->body,"mute_brightness",&mb);if(field<0){err(r,400,LE_INVALID,"Button settings are invalid");return;}if(field>0){next_mute_brightness=mb;changed=1;}field=json_get_string(q->body,"short_press",short_action,sizeof(short_action));if(field<0){err(r,400,LE_INVALID,"Button settings are invalid");return;}if(field>0){snprintf(next_short,sizeof(next_short),"%s",short_action);changed=1;}field=json_get_string(q->body,"long_press",long_action,sizeof(long_action));if(field<0){err(r,400,LE_INVALID,"Button settings are invalid");return;}if(field>0){snprintf(next_long,sizeof(next_long),"%s",long_action);changed=1;}if(!changed){err(r,400,LE_INVALID,"Button settings are invalid");return;}snprintf(old_short,sizeof(old_short),"%s",c->button_short);snprintf(old_long,sizeof(old_long),"%s",c->button_long);snprintf(old_action,sizeof(old_action),"%s",c->button_action);snprintf(old_sounds,sizeof(old_sounds),"%s",c->button_action_sounds);c->button_tones=next_tones;snprintf(c->button_action,sizeof(c->button_action),"%s",next_action);snprintf(c->button_action_sounds,sizeof(c->button_action_sounds),"%s",next_sounds);c->button_action_brightness=next_action_brightness;c->button_mute_brightness=next_mute_brightness;snprintf(c->button_short,sizeof(c->button_short),"%s",next_short);snprintf(c->button_long,sizeof(c->button_long),"%s",next_long);rc=persist_configuration(c);if(rc){c->button_tones=old_tones;c->button_action_brightness=old_ab;c->button_mute_brightness=old_mb;snprintf(c->button_short,sizeof(c->button_short),"%s",old_short);snprintf(c->button_long,sizeof(c->button_long),"%s",old_long);snprintf(c->button_action,sizeof(c->button_action),"%s",old_action);snprintf(c->button_action_sounds,sizeof(c->button_action_sounds),"%s",old_sounds);err(r,503,rc,"Button settings could not be saved");return;}}buttons_json(c,r);return;}
 ...
 if(!strcmp(p,"/api/v1/system/boot")){if(strcmp(q->method,"GET")){method_not_allowed(r);return;}boot_control_json(r);return;}
