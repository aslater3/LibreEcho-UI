 ...
 */
 if(!strcmp(p,"/api/v1/system/features")){
  ...
  if(!strcmp(q->method,"PUT")){int host,av,hv,sim,want_host=json_get_bool(q->body,"usb_host",&host),want_https=json_get_bool(q->body,"https",&hv),want_sim=json_get_bool(q->body,"simulation",&sim),want_aed=json_get_bool(q->body,"acoustic_events",&av);
