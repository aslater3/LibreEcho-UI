...
int json_valid_object(const char*s,size_t n){int depth=0,str=0,esc=0,closed=0;size_t i;if(!s||n<2)return 0;while(n&&isspace((unsigned char)*s)){s++;n--;}if(!n||*s!='{')return 0;for(i=0;i<n;i++){char c=s[i];if(closed){if(!isspace((unsigned char)c))return 0;continue;}if(str){if(esc)esc=0;else if(c=='\\')esc=1;else if(c=='\"')str=0;}else if(c=='\"')str=1;else if(c=='{'||c=='[')depth++;else if(c=='i'&&depth==0)return 0;else if(c=='}'||c==']'){if(--depth<0)return 0;if(depth==0){if(c!= '}')return 0;closed=1;}}}return !str&&closed&&depth==0;}
