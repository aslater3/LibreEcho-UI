...
static int run_update_fetch(int fd,const char*action){char success[160];int status,n;pid_t child=fork();if(child<0){update_error(fd,503,"io_error","The update command could not start");close(fd);return 0;}if(child==0){execl("/usr/local/sbin/libreecho-update-fetch","libreecho-update-fetch",action,(char*)0);_exit(127);}if(waitpid(child,&status,0)<0||!WIFEXITED(status)||WEXITSTATUS(status)!=0){int channel_action=!strncmp(action,"set-channel-",12);update_error(fd,channel_action?503:(!strcmp(action,"check")?503:400),channel_action?"io_error":(!strcmp(action,"check")?"update_check_failed":"update_rejected"),channel_action?"The update channel could not be changed":(!strcmp(action,"check")?"The GitHub release check failed":"The signed update failed verification or installation"));close(fd);return 0;}if(!strncmp(action,"set-channel-",12)){n=snprintf(success,sizeof(success),"{\"ok\":true,\"data\":{\"channel\":\"%s\"},\"error\":null}",action+12);}else{n=snprintf(success,sizeof(success),"{\"ok\":true,\"data\":{\"checked\":%s,\"installed\":%s,\"state\":\"%s\"},\"error\":null}",!strcmp(action,"check")?"true":"false",!strcmp(action,"install")?"true":"false",!strcmp(action,"check")?"checked":"reboot-pending");}response(fd,200,"application/json",success,(size_t)n);close(fd);return 0;}
