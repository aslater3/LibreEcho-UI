...
static void close_kernel_log_inherited(int keep_fd,int http_listener,int https_listener,int relay_listener,const struct client*clients,int max){int i;if(http_listener>=0&&http_listener!=keep_fd)close(http_listener);if(https_listener>=0&&https_listener!=keep_fd)close(https_listener);if(relay_listener>=0&&relay_listener!=keep_fd)close(relay_listener);for(i=0;i<max;i++)if(clients[i].fd>=0&&clients[i].fd!=keep_fd)close(clients[i].fd);}
static int start_kernel_log_stream(int fd,int http_listener,int https_listener,int relay_listener,const struct client*clients,int max){sigset_t blocked,previous;pid_t pid;int i,slot=-1;sigemptyset(&blocked);sigaddset(&blocked,SIGCHLD);if(sigprocmask(SIG_BLOCK,&blocked,&previous)<0)return-1;for(i=0;i<LE_MAX_KERNEL_LOG_WORKERS;i++)if(kernel_log_pids[i]<=0){slot=i;break;}if(slot<0){sigprocmask(SIG_SETMASK,&previous,NULL);return-1;}pid=fork();if(pid<0){sigprocmask(SIG_SETMASK,&previous,NULL);return-1;}if(pid==0){sigprocmask(SIG_SETMASK,&previous,NULL);close_kernel_log_inherited(fd,http_listener,https_listener,relay_listener,clients,max);(void)stream_kernel_log(fd);_exit(0);}kernel_log_pids[slot]=pid;kernel_log_workers++;sigprocmask(SIG_SETMASK,&previous,NULL);close(fd);return 0;}
