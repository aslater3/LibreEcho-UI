...
static int stream_pcm(int fd,int card,int device,int channels,int bits,int selected_channel){int pipe_fds[2],capture;char card_text[16],device_text[16],channels_text[16],bits_text[16],buffer[8192],header[512];ssize_t n;char*argv[17];if(pipe(pipe_fds)<0)return-1;capture=fork();if(capture<0){close(pipe_fds[0]);close(pipe_fds[1]);return-1;}if(capture==0){snprintf(card_text,sizeof(card_text),"%d",card);snprintf(device_text,sizeof(device_text),"%d",device);snprintf(channels_text,sizeof(channels_text),"%d",channels);snprintf(bits_text,sizeof(bits_text),"%d",bits);close(pipe_fds[0]);if(dup2(pipe_fds[1],STDOUT_FILENO)<0)_exit(127);close(pipe_fds[1]);argv[0]="tinycap";argv[1]="--";argv[2]="-D";argv[3]=card_text;argv[4]="-d";argv[5]=device_text;argv[6]="-c";argv[7]=channels_text;argv[8]="-r";argv[9]="16000";argv[10]="-b";argv[11]=bits_text;argv[12]="-p";argv[13]="1024";argv[14]="-n";argv[15]="4";argv[16]=NULL;execv("/sbin/tinycap",argv);_exit(127);}close(pipe_fds[1]);signal(SIGPIPE,SIG_IGN);n=(ssize_t)snprintf(header,sizeof(header),"HTTP/1.1 200 OK\r\nContent-Type: application/octet-stream\r\nTransfer-Encoding: chunked\r\nConnection: close\r\nCache-Control: no-store\r\nX-LibreEcho-Audio: pcm_s24_3le;rate=16000;channels=%d;selected-channel=%d\r\nX-Content-Type-Options: nosniff\r\n\r\n",channels,selected_channel);if(n<0||stream_send_all(fd,header,(size_t)n)<0)goto stop;while((n=read(pipe_fds[0],buffer,sizeof(buffer)))>0){int z=snprintf(header,sizeof(header),"%zx\r\n",(size_t)n);if(z<0||stream_send_all(fd,header,(size_t)z)<0||stream_send_all(fd,buffer,(size_t)n)<0||stream_send_all(fd,"\r\n",2)<0)goto stop;}if(n==0)stream_send_all(fd,"0\r\n\r\n",5);stop:close(pipe_fds[0]);kill(capture,SIGTERM);(void)waitpid(capture,NULL,0);close(fd);return 0;}
static int start_pcm_stream(int fd,int card,int device,int channels,int bits,int selected_channel){pid_t pid=fork();if(pid<0)return-1;if(pid==0){(void)stream_pcm(fd,card,device,channels,bits,selected_channel);_exit(0);}return 0;}
