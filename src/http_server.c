...
else {const char*base=strrchr(q.path,'/');base=base?base+1:q.path;page_path=q.path;if(!strcmp(q.path,"/login"))page_path="/login.html";else if(!strcmp(q.path,"/initial-setup"))page_path="/initial-setup.html";else if(!api->setup_completed)page_path="/setup.html";else if(api_bootstrap_required(api)&&!strchr(base,'.'))page_path="/initial-setup.html";if(serve_file(c->fd,o,page_path)){if(strchr(base,'.')||serve_file(c->fd,o,"/index.html"))response(c->fd,404,"text/plain","Not found",9);}}
