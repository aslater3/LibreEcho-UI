...
if(!strcmp(q.path,"/api/v1/system/update/channel")){const char*action=strstr(q.body,"\"channel\":\"dev\"")?"set-channel-dev":"set-channel-stable";if(!api_update_channel_authorize(api,&q,&r)){response(c->fd,r.status,r.type,r.body,r.length);goto done;}if(start_update_fetch(c->fd,action)<0){update_error(c->fd,503,"io_error","The update channel could not be changed");goto done;}c->fd=-1;c->used=0;return;}
