...
static int stream_kernel_log(int fd){FILE*f=popen("dmesg 2>/dev/null","r");char buf[8192],head[512];int z;size_t n;if(!f){const char*m="{\"ok\":false,\"data\":null,\"error\":{\"code\":\"io\",\"message\":\"Kernel log is unavailable\"}}";response(fd,503,"application/json",m,strlen(m));return 0;}signal(SIGPIPE,SIG_IGN);z=snprintf(head,sizeof(head),"HTTP/1.1 200 OK\r\nContent-Type: text/plain; charset=utf-8\r\nContent-Disposition: attachment; filename=\"libreecho-kernel.log\"\r\nTransfer-Encoding: chunked\r\nConnection: close\r\nCache-Control: no-store\r\nX-Content-Type-Options: nosniff\r\n\r\n");if(z<0||stream_send_all(fd,head,(size_t)z)<0){pclose(f);return 0;}while((n=fread(buf,1,sizeof(buf),f))>0){z=snprintf(head,sizeof(head),"%zx\r\n",n);if(z<0||stream_send_all(fd,head,(size_t)z)<0||stream_send_all(fd,buf,n)<0||stream_send_all(fd,"\r\n",2)<0){pclose(f);return 0;}}(void)stream_send_all(fd,"0\r\n\r\n",5);pclose(f);return 0;}
static int start_kernel_log_stream(int fd){pid_t pid=fork();if(pid<0)return-1;if(pid==0){(void)stream_kernel_log(fd);_exit(0);}close(fd);return 0;}
