...
#define LE_UPDATE_ERRLOG "/tmp/libreecho-update-install.err"
...
/* Read the last ERROR: token out of the captured stderr.  Only [a-z0-9_-] is
   copied, so nothing the child prints can break out of the JSON string. */
static void update_failure_reason(char*out,size_t cap){int f;ssize_t n;char buf[2048],*p,*last=0;size_t i=0;if(cap)out[0]='\0';f=open(LE_UPDATE_ERRLOG,O_RDONLY|O_CLOEXEC);if(f<0)return;n=read(f,buf,sizeof(buf)-1);close(f);if(n<=0)return;buf[n]='\0';for(p=buf;(p=strstr(p,"ERROR:"));p+=6)last=p+6;if(!last)return;while(last[i]&&i+1<cap){char c=last[i];if((c>='a'&&c<='z')||(c>='0'&&c<='9')||c=='_'||c=='-'){out[i]=c;i++;}else break;}out[i]='\0';}
static int stream_update_upload(int fd,const char*initial,size_t initial_len,size_t content_len,int allow_unsigned){static const char success[]="{\"ok\":true,\"data\":{\"installed\":true,\"state\":\"reboot-pending\"},\"error\":null}";char buffer[16384];size_t received=initial_len;int out=-1,lock=-1,status;ssize_t n;pid_t child;if(mkdir("/data/libreecho",0700)&&errno!=EEXIST)goto io;if(mkdir("/data/libreecho/update",0700)&&errno!=EEXIST)goto io;if(mkdir("/data/libreecho/update/incoming",0700)&&errno!=EEXIST)goto io;lock=open(LE_UPDATE_LOCK,O_WRONLY|O_CREAT|O_EXCL|O_CLOEXEC,0600);if(lock<0){update_error(fd,409,"update_busy","Another update upload is active");goto done;}close(lock);lock=1;out=open(LE_UPDATE_TMP,O_WRONLY|O_CREAT|O_EXCL|O_CLOEXEC,0600);if(out<0)goto io;if(initial_len&&write_all_file(out,initial,initial_len))goto io;while(received<content_len){size_t want=content_len-received;if(want>sizeof(buffer))want=sizeof(buffer);n=recv(fd,buffer,want,0);if(n<=0)goto io;if(write_all_file(out,buffer,(size_t)n))goto io;received+=(size_t)n;}if(fsync(out)||close(out)){out=-1;goto io;}out=-1;if(rename(LE_UPDATE_TMP,LE_UPDATE_PATH))goto io;child=fork();if(child<0)goto io;if(child==0){int e=open(LE_UPDATE_ERRLOG,O_WRONLY|O_CREAT|O_TRUNC,0600);if(e>=0){dup2(e,STDERR_FILENO);close(e);}if(allow_unsigned)execl("/usr/local/sbin/libreecho-update","libreecho-update","install","--allow-unsigned",LE_UPDATE_PATH,(char*)0);else execl("/usr/local/sbin/libreecho-update","libreecho-update","install",LE_UPDATE_PATH,(char*)0);_exit(127);}if(waitpid(child,&status,0)<0||!WIFEXITED(status)||WEXITSTATUS(status)!=0){char reason[64];update_failure_reason(reason,sizeof(reason));update_error_reason(fd,400,"update_rejected",!strcmp(reason,"manifest_update_channel_mismatch")?"The update targets a different release channel than this device":"The signed update failed verification or installation",reason);goto done;}response(fd,200,"application/json",success,sizeof(success)-1);goto done;io:if(out>=0)close(out);update_error(fd,503,"io_error","The update upload could not be stored");done:unlink(LE_UPDATE_TMP);if(lock==1)unlink(LE_UPDATE_LOCK);close(fd);return 0;}
