...
async function timersPage(){
 ...
 content.innerHTML=`<div class="settings-grid">${panel('Timers',
  ...
  ${panel('New timer',`<label class="field"><span>Minutes</span><input id="timer-minutes" type="number" min="1" max="1440" value="10"></label><label class="field"><span>Name (optional)</span><input id="timer-label" type="text" maxlength="47" placeholder="pasta"></label><div class="button-row">${action('Start timer','add-timer','primary-btn')}</div><p class="muted">Timers can also be set by voice: “set a timer for ten minutes”.</p>`)}</div>`;
 $('#add-timer').onclick=()=>{
  const minutes=+$('#timer-minutes').value;
  if(!(minutes>=1)){toast('Enter a length of at least one minute',true);return}
  post('/timers',{seconds:Math.round(minutes*60),label:$('#timer-label').value||''},'Timer started')};
