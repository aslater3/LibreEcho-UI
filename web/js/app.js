...
function macRow(label,id,live,factory,configured){
 ...
 return `<dl class="facts"><dt>${esc(label)} in use</dt><dd class="mono${same?' connected':''}">${esc(live||'unknown')}</dd>`
  ...
  +(id==='wifi'?`<p class="muted">This adapter's address cannot be changed: the driver rejects it (<code>SIOCSIFHWADDR: Not supported</code>), and it is not exposed through NVRAM or a module parameter. The address in use is generated by the driver and differs between boots, which is why the device does not keep a stable DHCP lease.</p>`:field(`${label} address override`,configured||'',`mac-${id}`));
}
