...
async function systemPage(){
  ...
  $('#copy-diag').onclick=async()=>{try{const bundle=state.data.diagnosticBundle||await api('/diagnostics/export',{method:'POST',body:'{}'});state.data.diagnosticBundle=bundle;await navigator.clipboard.writeText(bundle.summary||'LibreEcho diagnostic summary unavailable');toast('Health summary copied')}catch(e){toast(e.message,true)}};
}
