...
async function integrationsPage(){const [d,a]=await Promise.all([api('/integrations'),api('/assistant').catch(e=>({unsupported:e.message}))]);const haEnabled=d.items.some(x=>x.id==='home-assistant'&&x.enabled);content.innerHTML=`<div class="integration-grid">${assistantCard(a,haEnabled)}${weatherCard(a)}${d.items.map(x=>collapsiblePanel(x.name,`<p class="muted">${x.id==='rest'?'Versioned local device API.':'Optional local integration; no cloud connection required.'}</p>${toggle('Enabled',x.enabled,'int-'+x.id)}<div class="status-line"><span class="status-dot ${x.enabled?'ok':''}"></span><span>${x.enabled?'Enabled':'Not configured'}</span></div>${saveButton('save-int-'+x.id)}`)).join('')}</div>`;d.items.forEach(x=>{bindDirty(['#int-'+x.id],'#save-int-'+x.id);$('#save-int-'+x.id).onclick=()=>mutate('/integrations/'+x.id,{enabled:$('#int-'+x.id).checked},x.name+' changes saved')});if(!a.unsupported&&!haEnabled&&$('#wx-provider')){$('#wx-location').value=a.home_location||'';$('#wx-lat').value=a.latitude||'';$('#wx-lon').value=a.longitude||'';bindDirty(['#wx-provider','#wx-location','#wx-lat','#wx-lon'],'#save-wx');$('#save-wx').onclick=()=>mutate('/assistant',{weather_provider:wxId($('#wx-provider').value),home_location:$('#wx-location').value.trim(),latitude:$('#wx-lat').value.trim(),longitude:$('#wx-lon').value.trim()},'Weather settings saved')}
if(!a.unsupported&&!haEnabled){bindDirty(['#assistant-enabled','#assistant-model','#assistant-prompt'],'#save-assistant');$('#save-assistant').onclick=()=>mutate('/assistant',{enabled:$('#assistant-enabled').checked,provider:'openai-codex',model:$('#assistant-model').value.trim(),prompt:$('#assistant-prompt').value.trim()},'Voice assistant settings saved');if($('#assistant-auth-start'))$('#assistant-auth-start').onclick=()=>assistantAction('/assistant/auth/start','ChatGPT device sign-in started');if($('#assistant-auth-poll'))$('#assistant-auth-poll').onclick=()=>assistantAction('/assistant/auth/poll','Sign-in status checked');if($('#assistant-logout'))$('#assistant-logout').onclick=()=>assistantAction('/assistant/logout','ChatGPT disconnected');if($('#assistant-test'))$('#assistant-test').onclick=()=>post('/assistant/respond',{text:$('#assistant-test-text').value},'Test response queued');if(a.auth_state==='waiting')state.timer=setTimeout(async()=>{if(state.page!=='Integrations')return;try{await api('/assistant/auth/poll',{method:'POST',body:'{}'});await integrationsPage()}catch(e){toast(e.message,true)}},3000)}}
async function copyTextCompat(text){
  if(navigator.clipboard&&window.isSecureContext){await navigator.clipboard.writeText(text);return;}
  const area=document.createElement('textarea');area.value=text;area.setAttribute('readonly','');area.style.position='fixed';area.style.opacity='0';document.body.appendChild(area);
  try{area.select();if(!document.execCommand('copy'))throw new Error('Clipboard access is unavailable')}
  finally{if(area.parentNode)area.parentNode.removeChild(area)}
}
function bootSlotPanel(boot={}){if(!boot||!boot.available)return '';const slot=String(boot.running_slot||'-').toUpperCase();const tries=(typeof boot.running_slot_tries==='number'&&boot.running_slot_tries>=0)?String(boot.running_slot_tries):'Unknown';const count=Number.isFinite(boot.boot_count)?boot.boot_count:0;if(boot.confirmed)return panel('Boot slot',`<dl class="facts"><dt>Confirmed slot</dt><dd>${esc(slot)}</dd><dt>Boots recorded</dt><dd>${count}</dd><dt>Service graph at first boot</dt><dd class="${boot.strict_graph?'connected':''}">${boot.strict_graph?'Complete':'Degraded'}</dd></dl><p class="muted">This slot is confirmed, so the bootloader keeps starting it. Recorded every boot; the history travels in the diagnostic bundle.</p>`);return panel('Boot slot',`<dl class="facts"><dt>Slot</dt><dd>${esc(slot)}</dd><dt>Confirmed</dt><dd>No</dd><dt>Bootloader retries left</dt><dd>${esc(tries)}</dd><dt>Boots recorded</dt><dd>${count}</dd><dt>Last check</dt><dd>${esc(boot.last_check||'unknown')}</dd></dl><p class="muted"><strong>Action needed.</strong> This slot has not been confirmed, and every boot consumes one of the bootloader's remaining retries. Once they run out the device stops starting and cannot be reached over ADB. Download the diagnostic bundle below <em>before</em> reinstalling, and reinstall the image if the retries are running low.</p>`,'wide')}
async function systemPage(){
  const [s,d,ota,provenance,boot]=await Promise.all([api('/system'),api('/device'),api('/system/update'),api('/provenance'),api('/system/boot').catch(()=>({available:false}))]);
