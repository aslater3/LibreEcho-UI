...
const state={page:'Overview',csrf:'',token:sessionStorage.getItem('libreecho-token')||'',username:sessionStorage.getItem('libreecho-username')||'',authMode:'development-disabled',timer:null,data:{},busy:false,features:{simulation:false},renderGeneration:0,timerViewReady:false};
...
function provenanceDisplay(value,fallback='Unavailable'){return value===null||value===undefined||value===''||value==='unavailable'?'Unavailable':String(value)}
function provenanceStatusLabel(value){return ({idle:'No recent transaction','installed':'Installed','commit-pending':'Commit pending','reboot-required':'Restart required',pending:'Pending','rolled-back':'Rolled back'})[value]||provenanceDisplay(value)}
function provenanceCapsuleDisplay(c){return c.runtime_capsule_sha256===null||c.runtime_capsule_sha256===undefined?(c.effective==='unavailable'?'Unavailable':'Not used'):provenanceDisplay(c.runtime_capsule_sha256)}
function provenanceCandidateDisplay(c){return c.candidate_kind==='unavailable'&&c.candidate_status==='missing'?' · No pending candidate':c.candidate_status&&c.candidate_status!=='unavailable'?` · Candidate ${provenanceDisplay(c.candidate_kind)} ${provenanceDisplay(c.candidate_status)}`:''}
function provenanceStatus(c,p){const running=c.running_daemon_status==='running'?'Running':c.running_daemon_status==='not-running'?'Not running':'Daemon unavailable',effective={present:'Installed',mismatch:'Payload mismatch',missing:'Payload missing',unavailable:'Payload unavailable'}[c.effective]||'Payload unavailable',activation=c.activation==='reboot'?'Restart required':'No restart pending',transaction=c.last_transaction_result||p.last_transaction_result,candidate=provenanceCandidateDisplay(c);return `${effective} · ${running}${candidate} · ${activation} · ${provenanceStatusLabel(transaction)}`}
function provenanceCopy(label,value){return `<button type="button" class="secondary-btn copy-provenance" data-copy-label="${esc(label)}" data-copy-value="${esc(value)}" aria-label="Copy ${esc(label)}">Copy</button>`}
function provenanceTechnicalDetails(c){const source=provenanceDisplay(c.source_commit),effective=provenanceDisplay(c.effective_payload_sha256),capsule=provenanceCapsuleDisplay(c),candidate=c.candidate_kind==='unavailable'&&c.candidate_status==='missing'?'No pending candidate':provenanceDisplay(c.candidate_payload_sha256),candidateStatus=c.candidate_kind==='unavailable'&&c.candidate_status==='missing'?'No pending candidate':provenanceDisplay(c.candidate_status),candidateLabel=c.candidate_kind==='unavailable'&&c.candidate_status==='missing'?'Candidate':c.candidate_kind&&c.candidate_kind!=='unavailable'?`Candidate (${c.candidate_kind}) SHA-256`:'Candidate SHA-256',running=provenanceDisplay(c.running_daemon_sha256);return `<details class="provenance-details"><summary>Technical details</summary><dl class="facts provenance-facts"><dt>Source commit</dt><dd><code>${esc(source)}</code>${provenanceCopy('source commit',source)}</dd><dt>Effective payload SHA-256</dt><dd><code>${esc(effective)}</code>${provenanceCopy('effective payload hash',effective)}</dd><dt>Runtime capsule SHA-256</dt><dd><code>${esc(capsule)}</code>${provenanceCopy('runtime capsule hash',capsule)}</dd><dt>${esc(candidateLabel)}</dt><dd><code>${esc(candidate)}</code>${provenanceCopy('candidate payload hash',candidate)}</dd><dt>Candidate status</dt><dd>${esc(candidateStatus)}</dd><dt>Running daemon SHA-256</dt><dd><code>${esc(running)}</code>${provenanceCopy('running daemon hash',running)}</dd></dl></details>`}
function provenanceIdentityDetails(p){const source=provenanceDisplay(p.source_commit),digest=provenanceDisplay(p.source_digest);return `<details class="provenance-identity"><summary>Build identity</summary><dl class="facts provenance-facts"><dt>OS version</dt><dd>${esc(provenanceDisplay(p.os_version))}</dd><dt>Source commit</dt><dd><code>${esc(source)}</code>${provenanceCopy('build source commit',source)}</dd><dt>Source digest</dt><dd><code>${esc(digest)}</code>${provenanceCopy('build source digest',digest)}</dd><dt>Transaction state</dt><dd>${esc(provenanceDisplay(p.transaction_state))}</dd><dt>Last transaction result</dt><dd>${esc(provenanceDisplay(p.last_transaction_result))}</dd></dl></details>`}
function bindProvenanceCopy(){ $$('.copy-provenance',content).forEach(button=>{button.onclick=async()=>{const label=button.dataset.copyLabel||'Value',original=button.textContent;button.disabled=true;try{await copyTextCompat(button.dataset.copyValue||'');button.textContent='Copied';toast(`${label} copied`)}catch(error){button.textContent='Copy failed';toast(`Copy failed: ${error.message||'clipboard access is unavailable'}`,true)}finally{setTimeout(()=>{button.disabled=false;button.textContent=original},1400)}}})}
function provenanceAuthorityDetails(p){const a=p.authority_provenance||{};if(!a.available)return `<details class="provenance-details provenance-authority"><summary>Signed authorizing authority</summary><p class="muted">Unavailable. The UI does not infer signed release/source identity from mutable records.</p></details>`;const rows=(Array.isArray(a.features)?a.features:[]).map(f=>`<tr><th scope="row">${esc(provenanceDisplay(f.feature_id))}</th><td>${esc(provenanceDisplay(f.release))}</td><td>${esc(provenanceDisplay(f.source_commit))}</td><td>${esc(provenanceDisplay(f.kind))}</td></tr>`).join('');return `<details class="provenance-details provenance-authority"><summary>Signed authorizing authority</summary><p class="muted">This release/source identity authorizes the effective bytes observed by Platform. It may differ from the original build provenance above.</p><dl class="facts provenance-facts"><dt>Transaction</dt><dd><code>${esc(provenanceDisplay(a.transaction_id))}</code></dd><dt>Manifest SHA-256</dt><dd><code>${esc(provenanceDisplay(a.manifest_sha256))}</code></dd><dt>Manifest signature SHA-256</dt><dd><code>${esc(provenanceDisplay(a.manifest_sig_sha256))}</code></dd></dl>${rows?`<div class="provenance-table-wrap"><table class="data-table provenance-authority-table"><caption class="sr-only">Signed authorizing release and source</caption><thead><tr><th scope="col">Feature</th><th scope="col">Authorizing release</th><th scope="col">Authorizing source</th><th scope="col">Kind</th></tr></thead><tbody>${rows}</tbody></table></div>`:''}</details>`}
function provenancePanel(p={}){const components=Array.isArray(p.components)?p.components:[],rows=components.map(c=>`<tr><th scope="row">${esc(provenanceDisplay(c.feature_id))}</th><td>${esc(provenanceDisplay(c.release))}</td><td>${esc(provenanceStatus(c,p))}</td><td>${provenanceTechnicalDetails(c)}</td></tr>`).join('');return panel('Feature provenance',`<p class="muted">Observed component identity only; missing legacy identity is unavailable and releases are never inferred.</p>${provenanceAuthorityDetails(p)}${rows?`<div class="provenance-table-wrap"><table class="data-table provenance-table"><caption class="sr-only">Feature release and runtime status</caption><thead><tr><th scope="col">Component</th><th scope="col">Release</th><th scope="col">Status</th><th scope="col">Details</th></tr></thead><tbody>${rows}</tbody></table></div>`:'<p class="muted">No component identity is available.</p>'}${provenanceIdentityDetails(p)}`,'wide')}
...
/*
 * The action button is a sound machine: one curated list, and each press plays
 * the next entry. The list is stored as a comma-separated string of sound
 * names in play order -- the order the rows appear is the order they rotate --
 * because buttond re-reads it from the config file on every tick, and a flat
 * string is the cheapest thing for it to parse.
 *
 * Sounds come from whatever is present in the device's sound directory rather
 * than a hard-coded list, so dropping a new .raw in makes it selectable here
 * without a UI change.
 */
function actionSoundPanel(b){
 const available=Array.isArray(b.available_sounds)?b.available_sounds:[];
 const enabled=(b.action_sounds||'').split(',').map(x=>x.trim()).filter(Boolean);
 /* Enabled first, in rotation order, then the rest: the list IS the running
    order, so showing it in any other order would misdescribe it. */
 const ordered=enabled.filter(n=>available.includes(n))
   .concat(available.filter(n=>!enabled.includes(n)));
 const rows=ordered.length?ordered.map(name=>{
   const on=enabled.includes(name);
   const pos=on?enabled.indexOf(name)+1:0;
   return `<label class="sound-row${on?' on':''}"><input type="checkbox" data-sound="${esc(name)}"${on?' checked':''}>`+
     `<span class="sound-name">${esc(name)}</span>`+
     `<span class="sound-pos">${pos?'#'+pos:'off'}</span>`+
     `<button class="secondary-btn sound-try" data-try="${esc(name)}">Play</button></label>`;
 }).join(''):'<p class="muted">No sounds are installed on this device.</p>';
 return panel('Action button sounds',
   `<p class="muted">Each press plays the next sound that is switched on, then starts again. Switch them all off and the button just flashes the ring.</p>`+
   `<div class="sound-list">${rows}</div>`+
   `<p class="muted" id="ab-count"></p>`,'action-sound-panel');
}
function bindActionSounds(){
 const boxes=()=>$$('#content input[data-sound]');
 const countLine=()=>{
  const n=boxes().filter(x=>x.checked).length;
  const out=$('#ab-count');
  if(out)out.textContent=n?`${n} sound${n===1?'':'s'} in the rotation.`
    :'No sounds selected — the button will flash the ring and stay silent.';
 };
 const positions=()=>{
  let i=0;
  boxes().forEach(x=>{
   const row=x.closest('.sound-row'),tag=row&&row.querySelector('.sound-pos');
   if(row)row.classList.toggle('on',x.checked);
   if(tag)tag.textContent=x.checked?'#'+(++i):'off';
  });
 };
...
$$('.profile-save').forEach(button=>button.onclick=()=>{const k=button.dataset.profile;const h=$(`#led-profile-${k}`).value;const brightnessInput=$(`#led-profile-brightness-${k}`);mutate('/led/profile',{name:k,r:parseInt(h.slice(1,3),16),g:parseInt(h.slice(3,5),16),b:parseInt(h.slice(5,7),16),brightness:+brightnessInput.value},`${names[k]||k} theme saved`)});$('#save-buttons').onclick=()=>mutate('/buttons',{short_press:$('#short-action').value,long_press:$('#long-action').value,tones:$('#button-tones').checked,action:{'Play a sound':'sound','Start listening':'listen','Play / pause':'playpause','Do nothing':'disabled'}[$('#action-behaviour').value]||'sound',action_sounds:actionSoundSelection(),action_brightness:parseInt($('#action-brightness').value,10),mute_brightness:parseInt($('#mute-brightness').value,10)},'Button changes saved');/* The ring test paints at the ring's own brightness, so with brightness at 0
     ...
     button, and did. Gate it on the saved brightness rather than on the
     slider: the slider is not applied until Save, so enabling the button the
     moment it is dragged would promise a test the device still cannot show. */
  {const testBtn=$('#led-test');
   if(testBtn){
    if(!(l.brightness>0)){
...
/* The Local LLM panel and the ChatGPT panel are two views of one setting:
 * agentd stores a single provider, model, base_url and api_key. Saving from
 * either panel switches the active provider, so each says so plainly. The API
 * key is write-only -- agentd reports api_key_configured and never returns the
 ...
 * entirely rather than sent empty, which agentd would read as a clear. */
if($('#save-local'))$('#save-local').onclick=()=>{const url=$('#local-base-url').value.trim(),model=$('#local-model').value.trim(),key=$('#local-api-key').value;if(!/^https?:\/\//.test(url))return toast('Endpoint URL must start with http:// or https://',true);if(!model)return toast('Enter the model name that endpoint serves',true);const body={enabled:$('#local-enabled').checked,provider:'openai-compatible',base_url:url,model:model};if(key)body.api_key=key;return mutate('/assistant',body,'Local LLM settings saved')};
if($('#local-test'))$('#local-test').onclick=()=>post('/assistant/respond',{text:'Say hello in one short sentence.'},'Test response queued');
if($('#local-clear-key'))$('#local-clear-key').onclick=()=>mutate('/assistant',{api_key:''},'API key cleared');if($('#assistant-auth-start'))$('#assistant-auth-start').onclick=()=>assistantAction('/assistant/auth/start','ChatGPT device sign-in started');if($('#assistant-auth-poll'))$('#assistant-auth-poll').onclick=()=>assistantAction('/assistant/auth/poll','Sign-in status checked');if($('#assistant-logout'))$('#assistant-logout').onclick=()=>assistantAction('/assistant/logout','ChatGPT disconnected');if($('#assistant-test'))$('#assistant-test').onclick=()=>post('/assistant/respond',{text:$('#assistant-test-text').value},'Test response queued');if(a.auth_state==='waiting')state.timer=setTimeout(async()=>{if(state.page!=='Integrations')return;try{await api('/assistant/auth/poll',{method:'POST',body:'{}'});await integrationsPage()}catch(e){toast(e.message,true)}},3000)}}
...
/*
 * The identity of the candidate an available update would install: the
 * immutable release tag the check resolved and the OTA digest the device
 * verifies the download against. The check record carries them only when a
 * completed check actually resolved an immutable candidate, so each is
 * described only when it is present. An older device, a stable candidate, a
 * failed check or a check that has not run leaves the version-only message
 * standing rather than implying an identity the device never resolved.
 */
function updateIdentity(ota){
 const tag=String(ota&&ota.resolved_release_tag||'').trim();
 const digest=String(ota&&ota.ota_sha256||'').trim();
 return {tag,digest,text:[tag?'release '+tag:'',digest?'SHA-256 '+digest:''].filter(Boolean).join(' · ')};}
async function systemPage(){
  const [s,d,ota,features,provenance]=await Promise.all([api('/system'),api('/device'),api('/system/update'),api('/system/features').catch(e=>({unsupported:e.message})),api('/provenance')]);
  ...
  const featuresPanel=panel('Features',features.unsupported?unsupported(features.unsupported):
    ...
    `<p class="muted">The setting is temporary and resets to device mode on the next boot, keeping ADB available as the recovery path. Applies immediately &mdash; the Save button below is for Simulation only.</p>`+
  ...
  /* The role is a live hardware switch, deliberately outside the Save flow.
     It resets to device mode on reboot so ADB remains the recovery path. */
  if($('#feature-usb-host'))$('#feature-usb-host').onchange=()=>{
   ...
   if(el.checked&&!confirm('Switch the USB port to storage mode? (experimental)\n\n'
     ...
     +'ADB does stop while host mode is active, because the port cannot read a drive and serve ADB at once. '
     +'The setting is temporary and resets to device mode on the next boot.')){
...
async function logsPage(){const [l,d]=await Promise.all([api('/logs'),api('/diagnostics')]);const clock=l.entries.some(x=>Number(x.timestamp)<1577836800)?'Device clock unset; relative boot time is shown where available':'Device clock available';content.innerHTML=`${panel('Service logs',`<div class="log-toolbar"><input id="log-filter" placeholder="Filter logs" aria-label="Filter logs">${action('Refresh','log-refresh')}${action('Download kernel log','kernel-log-download')}</div><div class="log-viewer" id="log-viewer">${l.entries.length?l.entries.map(x=>`<div data-log="${esc(x.message.toLowerCase())}"><time>${logTime(x.timestamp,x.boot_seconds)}</time><span class="level ${x.level}">${x.level}</span><span>${esc(x.message)}</span></div>`).join(''):'<p class="muted">No logs available.</p>'}</div><p class="muted">Source: ${esc(l.source||'web-memory')}; bounded to ${l.capacity} entries. ${clock}. Relative time is from boot; secrets are redacted before logging.</p>`)}<div class="settings-grid diagnostics">${diagnosticsPanel(d)}${panel('Mock fault injection',state.data.status?.simulated?`<p class="muted">Use <code>tools/mockctl.sh</code> with <code>--dev-controls</code> to inject deterministic faults.</p>${action('Fail next Wi-Fi scan','fail-wifi','danger-btn')}`:unsupported('Fault injection is only available in mock development builds.'))}</div>`;$('#log-filter').oninput=e=>$$('[data-log]').forEach(x=>x.hidden=!x.dataset.log.includes(e.target.value.toLowerCase()));$('#log-refresh').onclick=render;$('#kernel-log-download').onclick=async()=>{try{const res=await fetch('/api/v1/diagnostics/kernel.log',{headers:{...(state.token?{'Authorization':'Bearer '+state.token}:{})}});if(!res.ok)throw new Error('Kernel log unavailable ('+res.status+')');const text=await res.text();const blob=new Blob([text],{type:'text/plain'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='libreecho-kernel-'+new Date().toISOString().replace(/[:.]/g,'-')+'.log';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);toast('Kernel log downloaded ('+(text?text.split(String.fromCharCode(10)).length-1:0)+' lines)')}catch(e){toast(e.message,true)}};if($('#fail-wifi'))$('#fail-wifi').onclick=()=>post('/dev/mock',{action:'fail-next',value:'wifi-scan'},'Next Wi-Fi scan will fail')}
async function aboutPage(){const p=await api('/provenance').catch(()=>({os_version:'unavailable',source_commit:'unavailable',source_digest:'unavailable',transaction_state:'none',last_transaction_result:'unavailable',components:[]}));content.innerHTML=`<div class="settings-grid">${panel('LibreEcho',`<img class="about-mark" src="/assets/mark.svg" alt="LibreEcho mark"><p>Open source voice-assistant software built for privacy, repairability and local control.</p><dl class="facts"><dt>OS version</dt><dd>${esc(p.os_version||'unavailable')}</dd><dt>Web API</dt><dd>v1</dd><dt>Frontend</dt><dd>Dependency-free HTML, CSS and JavaScript</dd><dt>Daemon</dt><dd>Portable C99</dd><dt>Licence</dt><dd>MIT</dd></dl>`)}${provenancePanel(p)}${panel('Hardware independence',`<p class="muted">The same frontend API works with both the realistic mock backend and the conservative Linux hardware adapter.</p><div class="privacy-callout">Open. Private. Yours.</div>`)}${panel('Supported devices',`<p class="muted">Confirmed models have been booted and used with LibreEcho on real hardware. Likely models share the MT8163 platform and the Amonet exploit path, but have not been tried — treat them as untested.</p>`+specTable(['Device','Codename','SoC','Status'],[ ['Echo (2nd generation)','radar_puffin','MT8163','Confirmed'],['Echo Dot (2nd generation)','biscuit_puffin','MT8163','Confirmed'],['Echo Plus (1st generation)','knight_puffin','MT8163','Likely, untested'],['Echo Show 5 (1st generation)','crown_puffin','MT8163','Likely, untested'],['Echo Spot','pumpkin_puffin','MT8163','Likely, untested'],['Echo Input','gemini_puffin','MT8163','Likely, untested']])+`<p class="muted">Nothing outside this list is supported. Other Echo generations use different SoCs and have no known install path, so LibreEcho cannot be put on them today.</p>`)}</div>`;bindProvenanceCopy()}
/*
 * Timers.
 *
