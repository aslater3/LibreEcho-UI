...
function provenanceStatus(c,p){const running=c.running_daemon_status==='running'?'Running':c.running_daemon_status==='not-running'?'Not running':'Daemon unavailable',effective={present:'Installed',mismatch:'Payload mismatch',missing:'Payload missing',unavailable:'Payload unavailable'}[c.effective]||'Payload unavailable',activation=c.activation==='reboot'?'Restart required':'No restart pending',transaction=c.last_transaction_result||p.last_transaction_result,candidate=c.candidate_status&&c.candidate_status!=='unavailable'?` · Candidate ${provenanceDisplay(c.candidate_kind)} ${provenanceDisplay(c.candidate_status)}`:'';return `${effective} · ${running}${candidate} · ${activation} · ${provenanceStatusLabel(transaction)}`}
