...
Path('build/update-stderr.txt').write_text('x'*70000+'ERROR:current_slot_not_confirmed\\n')
PY
...
assert 'pipe(errpipe)' in c and 'sizeof(stderr_tail)' in c
...
assert 'ListeningSettings' in openapi and 'max_utterance_ms' in openapi
