...
[ "$code" = 403 ]
code=$(curl -sS -o /tmp/le-user-loopback-suffix.out -w '%{http_code}' -X POST "$URL/api/v1/auth/login" -H "$CSRF" -H 'Origin: http://localhost.evil.test' -H 'Content-Type: application/json' --data '{"username":"test-user","password":"test-password-123"}')
[ "$code" = 403 ]
code=$(curl -sS -o /tmp/le-user-loopback-ip-suffix.out -w '%{http_code}' -X POST "$URL/api/v1/auth/login" -H "$CSRF" -H 'Origin: https://127.0.0.1.attacker.test' -H 'Content-Type: application/json' --data '{"username":"test-user","password":"test-password-123"}')
[ "$code" = 403 ]
